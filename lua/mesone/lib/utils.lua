...
local M = {
  ...
  read_all = function(filename)
    return _read_all(filename)
  end,

  read_file = function(filename, callback)
    _read_file(filename, callback)
  end,

  read_json_file = function(filename)
    return vim.json.decode(_read_all(filename) or "null", { luanil = { object = true, array = true } })
  end,
  ...
  buf_append_colorized = function(buf, content, content_type, reset)
    ...
    local row_start = vim.api.nvim_buf_line_count(buf)
    ...
    if type(content) == "table" then
      vim.api.nvim_buf_set_lines(buf, start_line, -1, true, content)
    else
      vim.api.nvim_buf_set_lines(buf, start_line, -1, true, { content })
    end

    local row_end = vim.api.nvim_buf_line_count(buf)
    ...
    elseif content_type == "start" or content_type == "end" then
      highlight = "Title"
    elseif content_type == "target" then
      highlight = "Directory"
    end
    for row = row_start, row_end - 1 do
      vim.api.nvim_buf_add_highlight(buf, -1, highlight, row, 0, -1)
    end
    return row_end
  end,
  ...
  is_failure_message = function(str)
    return string.match(str:lower(), "error") or string.match(str:lower(), "failed")
  end,
}
